...
// This moves the student from the "Suspended" sector back to "Active"
if (isset($_GET['id']) && isset($_GET['action']) && $_GET['action'] == 'restore') {
    $id = mysqli_real_escape_string($conn, $_GET['id']);
    $restore_sql = "UPDATE students SET status = 'Active' WHERE id = '$id'";
    
    if (mysqli_query($conn, $restore_sql)) {
        // Redirect to the main list; student appears in their original ID spot
        header("Location: student-list.php?msg=restored");
        exit();
    }
}

// ✅ STEP 2: Fetch ONLY Suspended Students
// This query ensures only archived data reflects in this 10-column table
$query = "SELECT * FROM students WHERE status = 'Suspended' ORDER BY id ASC";
...
<body>
        ...
        <div class="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
          <div class="">
            ...
            <nav>
              ...
              <span class="text-secondary-light">/ Suspended List</span>
            </nav>
          </div>
        </div>
              ...
              <div class="d-flex flex-wrap align-items-center gap-16">
                <div class="dropdown">
                  ...
                  <ul class="dropdown-menu p-12 border bg-base shadow">
                    <li><button type="button" class="dropdown-item px-16 py-8 rounded text-secondary-light bg-hover-neutral-200">PDF</button></li>
                    <li><button type="button" class="dropdown-item px-16 py-8 rounded text-secondary-light bg-hover-neutral-200">Excel</button></li>
                  </ul>
                </div>

                <form class="navbar-search dt-search m-0">
                  <input type="text" id="customSearch" class="dt-input bg-transparent radius-4" placeholder="Search...">
                  <iconify-icon icon="ion:search-outline" class="icon"></iconify-icon>
                </form>

                <div class="dropdown">
                  <button type="button" class="btn btn-outline-neutral-600 dropdown-toggle" data-bs-toggle="dropdown">
                    Filter By
                  </button>
                  <ul class="dropdown-menu p-12 shadow">
                    <li><a class="dropdown-item" href="#">Search by Name</a></li>
                    <li><a class="dropdown-item" href="#">Search by Mobile</a></li>
                  </ul>
                </div>
              </div>
            </div>
            <div class="table-responsive">
              <table class="table bordered-table mb-0" id="dataTable">
                ...
                <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          <div class="d-flex align-items-center">
                            ...
                            <div>
                              <h6 class="text-md mb-0 fw-medium"><?= htmlspecialchars($row['full_name'] ?? '') ?></h6>
                        ...
                        <td>
                          <span class="badge <?= ($row['hostel_required'] == 'Yes') ? 'bg-success-100 text-success-600' : 'bg-neutral-100 text-neutral-600' ?> px-16 py-4 radius-4 fw-medium text-sm">
                            <?= htmlspecialchars($row['hostel_required'] ?? 'No') ?>
                          </span>
                        </td>
                        ...
                        <td>
                          <div class="btn-group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-lg-end border p-12">
                              <li>
                                <a href="suspended-student.php?id=<?= $row['id'] ?>&action=restore" onclick="return confirm('Restore this student?')" class="dropdown-item rounded text-success bg-hover-neutral-200 d-flex align-items-center gap-2 py-6">
                                  <i class="ri-refresh-line"></i> Restore Student
                                </a>
                              </li>
                            </ul>
                          </div>
                  ...
                  <?php else: ?>
                    <tr>
                      <td colspan="10" class="text-center py-20">No suspended records found.</td>
                    </tr>
                  <?php endif; ?>
                </tbody>
              </table>
  ...
  <script>
    $(document).ready(function() {
        // Destroy existing instance to prevent re-initialization errors
        ...
        const tableInstance = $('#dataTable').DataTable({
            pageLength: 10,
            dom: 'rtp', // 'r' processing, 't' table, 'p' pagination
            ordering: true,
            // Customizes the empty table message properly
            language: {
                emptyTable: "No suspended records found.",
                paginate: {
                    next: '<i class="ri-arrow-right-s-line"></i>',
                    previous: '<i class="ri-arrow-left-s-line"></i>'
                }
            },
            // Prevents errors if a column is missing data
            columnDefs: [{
                "defaultContent": "-",
                "targets": "_all"
            }]
