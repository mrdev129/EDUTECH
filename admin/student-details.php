...
// 1. Fetch the Student Data based on the request type
if (isset($_POST['search_submit'])) {
    // Handle Search from Navbar
    $search_val = mysqli_real_escape_string($conn, $_POST['search_value']);
    $filter_by = $_POST['filter_by'];
    $sql = "SELECT * FROM students WHERE $filter_by LIKE '%$search_val%' LIMIT 1";
} elseif (isset($_GET['id'])) {
    // Handle Direct Link from Student List or Suspended List
    $id = mysqli_real_escape_string($conn, $_GET['id']);
    $sql = "SELECT * FROM students WHERE id = '$id'";
} else {
    // Default: Show the most recent record
...
// 2. ✅ Visibility Logic: Erase from view if status is 'Suspended'
if ($student) {
    // If student is suspended, redirect them away UNLESS the link comes from the suspended list
    if ($student['status'] == 'Suspended' && !isset($_GET['from_suspended'])) {
        header("Location: suspended-student.php?msg=already_suspended");
        exit();
    }
} else {
    // If no student is found at all, return to the list
    header("Location: student-list.php?msg=not_found");
    exit();
}
?>
            ...
            <div class="row align-items-center justify-content-between">
                <div class="col-auto">
                    <div class="d-flex flex-wrap align-items-center gap-4">
                        ...
                        <form class="navbar-search d-flex align-items-center gap-2 px-12 py-4 bg-neutral-50 radius-8" method="POST" action="student-details.php" style="border: 1px solid #e3e6e9; max-width: 400px;">

                            <button type="submit" name="search_submit" class="bg-transparent border-0 p-0 d-flex text-neutral-500">
                                <iconify-icon icon="ion:search-outline" style="font-size: 20px;"></iconify-icon>
                            </button>

                            <div class="d-flex align-items-center">
                                <select name="filter_by" class="form-select bg-transparent border-0 text-secondary-light fw-medium text-sm" style="box-shadow: none; cursor: pointer; width: 90px; padding-right: 20px;">
                            ...
                            <input type="text" class="bg-transparent border-0 flex-grow-1 ps-8" name="search_value" placeholder="Type here..." required style="outline: none; font-size: 14px;">
                        ...
                        <div class="d-flex gap-32 flex-md-row flex-column">
                            <div class="max-w-300-px w-100 text-center">
                                <figure class="mb-24 w-120-px h-120-px mx-auto rounded-circle overflow-hidden">
                                    <img src="assets/images/thumbs/student-details-img.png" alt="Student Image" class="w-100 h-100 object-fit-cover">
                                </figure>
                                <h2 class="h6 text-primary-light mb-16 fw-semibold">
                                    <?= htmlspecialchars($student['full_name']) ?>
                                </h2>
                                <p class="mb-0">Student ID: <span class="text-primary-600 fw-semibold">#<?= $student['id'] ?></span></p>
                                <div class="mt-32 d-flex gap-16 w-100">
                                    <a href="suspend-student-logic.php?id=<?= $student['id'] ?>"
                                        onclick="return confirm('Confirm suspension of <?= htmlspecialchars($student['full_name']) ?>?')"
                                        class="btn border border-danger-600 bg-hover-danger-200 text-danger-600 px-12 py-8 radius-8">
                                        <i class="ri-delete-bin-2-line"></i> Suspend
                                    </a>
                            ...
                            <div class="flex-grow-1">
                                <div class="pb-16 border-bottom d-flex align-items-center justify-content-between gap-20">
                                    <h3 class="h6 text-primary-light text-lg mb-0 fw-semibold">Admission Lead Data</h3>
                                    <span class="bg-success-100 text-success-600 px-16 py-4 radius-4 fw-medium text-sm">
                                        <?= htmlspecialchars($student['status'] ?? 'Active') ?>
                                    </span>
                                </div>

                                <div class="mt-16 d-flex flex-column gap-12">
                                    <div class="d-flex gap-4">
                                        <span class="fw-semibold text-sm text-primary-light w-150-px">Mobile Number</span>
                                        <span class="fw-normal text-sm text-primary-600">: <?= htmlspecialchars($student['mobile']) ?></span>
                                    </div>
                                    <div class="d-flex gap-4">
                                        <span class="fw-semibold text-sm text-primary-light w-150-px">Email Address</span>
                                        <span class="fw-normal text-sm text-primary-600">: <?= htmlspecialchars($student['email']) ?></span>
                                    </div>
                                    <div class="d-flex gap-4 pt-8 border-top">
                                        <span class="fw-semibold text-sm text-primary-light w-150-px">Last Qualification</span>
                                        <span class="fw-normal text-sm text-secondary-light">: <?= htmlspecialchars($student['last_qualification']) ?></span>
                                    </div>
                                    <div class="d-flex gap-4">
                                        <span class="fw-semibold text-sm text-primary-light w-150-px">Preferred Course</span>
                                        <span class="fw-normal text-sm text-secondary-light">: <?= htmlspecialchars($student['preferred_course']) ?></span>
                                    </div>
                                    <div class="d-flex gap-4">
                                        <span class="fw-semibold text-sm text-primary-light w-150-px">Preferred City</span>
                                        <span class="fw-normal text-sm text-secondary-light">: <?= htmlspecialchars($student['preferred_city']) ?></span>
                                    </div>
                                    <div class="d-flex gap-4">
                                        <span class="fw-semibold text-sm text-primary-light w-150-px">Budget Range</span>
                                        <span class="fw-normal text-sm text-secondary-light">: <?= htmlspecialchars($student['budget_range']) ?></span>
                                    </div>
                                    <div class="d-flex gap-4">
                                        <span class="fw-semibold text-sm text-primary-light w-150-px">Hostel Required</span>
                                        <span class="fw-normal text-sm text-secondary-light">: <?= htmlspecialchars($student['hostel_required']) ?></span>
                                    </div>
                                    <!-- <div class="mt-12">
            <span class="fw-semibold text-sm text-primary-light d-block mb-4">Message / Note:</span>
            <div class="bg-neutral-50 p-12 radius-8 border text-sm text-secondary-light">
                <?= nl2br(htmlspecialchars($student['message'])) ?>
            </div>
        </div> -->
                                </div>
                            </div>
        ...
        <div class="modal-dialog modal-sm modal-dialog modal-dialog-centered max-w-340-px">
            <div class="modal-content radius-16 bg-base">
                <div class="modal-body pt-32 px-36 pb-24 text-center">
                    ...
                    <h6 class="text-lg fw-semibold text-primary-light mb-0">Are your sure you want to Suspend this Student
                    </h6>
